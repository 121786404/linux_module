...
static int periods[PERIOD_THREAD_NUM] = {1,2,4,8,16};
...
{
        int k = (int)(long)data, count = -1;
        do {
                printk(KERN_INFO "thread%d: period=%ds, count=%d\n",
                       k, periods[k], ++count);
...
static int hello_init(void)
{
        ...
        printk(KERN_INFO "kthread-completion inserted\n");
        for (k = 0; k < PERIOD_THREAD_NUM; k++)
        ...
        for (k = 0; k < PERIOD_THREAD_NUM; k++) {
                period_tsks[k] = kthread_run(period_thread,
                                             (void *)(long)k,
                                            "period_thread%d", k);
...
{
        int k, count[PERIOD_THREAD_NUM], watchdog_count;

        printk(KERN_INFO "kthread-completion removed\n");
        for (k = 0; k < PERIOD_THREAD_NUM; k++) {
                count[k] = 0;
                if (!IS_ERR(period_tsks[k]))
        ...
        printk(KERN_INFO "running total time: %ds\n", watchdog_count);
        for (k = 0; k < PERIOD_THREAD_NUM; k++)
                printk(KERN_INFO "thread%d: period %d, running %d times\n",
                       k, periods[k], count[k]);
}
